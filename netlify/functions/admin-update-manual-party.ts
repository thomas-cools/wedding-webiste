...
import { updateManualParty, type ManualRsvpPartyGuest } from './utils/manual-rsvp-parties'

/**
 * Admin endpoint to edit a manually-added party.
 *
 * POST /api/admin-update-manual-party — Protected by admin JWT
 *
 * Body: { id: string; firstName: string; email: string; guests?: Array<{ name: string }> }
 */

const MAX_GUESTS = 30
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/

interface UpdateManualPartyBody {
  id?: unknown
  firstName?: unknown
  email?: unknown
...
export const handler: Handler = async (event) => {
  ...
  let body: UpdateManualPartyBody
  try {
  ...
  const { id, firstName, email, guests } = body
  ...
  try {
    const party = await updateManualParty(
      id.trim(),
      firstName.trim(),
    ...
    if (!party) {
      return adminJson(404, { ok: false, error: 'Party not found' })
    }
    return adminJson(200, { ok: true, party })
  } catch (error) {
    console.error('Failed to update manual party:', error)
    return adminJson(500, { ok: false, error: 'Failed to update party' })
  }
}
